...
Calendar.prototype._init = function (firstDayOfWeek, date) {
	...
	date.getLocalMonthDays(this.dateType);
	...
	(this.dateType == 'jalali' ? Calendar._JSMN[month] : Calendar._SMN[month]);
		...
		if (a[i].charAt(0) == '%') {
			switch (a[i]) {
			    ...
			    case "%B":
			    	var monthNames;
			    	if (dateType == 'jalali') {
			    		monthNames = (a[i] == '%b') ? Calendar._JSMN : Calendar._JMN;
			    	} else {
			    		monthNames = (a[i] == '%b') ? Calendar._SMN : Calendar._MN;
			    	}
					for (j = 0; j < 12; ++j) {
